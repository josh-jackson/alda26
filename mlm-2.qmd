...
b5_soep_long.item <- soep_long %>%
  filter(type == "Big 5") %>% 
  group_by(Procedural__SID, trait,item, year) %>% 
  dplyr::summarize(value = mean(value, na.rm = T)) %>% 
  ungroup() %>% 
  left_join(soep_long %>% 
    select(Procedural__SID, DOB = Demographic__DOB, Sex = Demographic__Sex) %>%
...
- Depending on your DV, you might not want to have a Gaussian sampling distribution. 

- One option is to average to get something semi-continuous. The other is to fit an IRT model. 
...
item_difficulties <- runif(n_items, -2, 2)
person_intercepts <- rnorm(n_people, mean = 0, sd = 1) 
person_slopes <- rnorm(n_people, mean = 0.5, sd = 0.2) 

df <- expand.grid(ID = 1:n_people, 
                  Time = 0:(n_waves-1), 
                  Item = 1:n_items) 
df <- df |>   
mutate(
    Ability = person_intercepts[ID] + (person_slopes[ID] * Time),
    Difficulty = item_difficulties[Item],
    Logit = Ability - Difficulty,
    Prob = plogis(Logit),
    Response = rbinom(8000, 1, Prob),
    Item_Factor = factor(Item)
  )

irt_growth_model <- glmer(
  Response ~ 0 + Item_Factor + Time + (Time | ID), 
  data = df, 
  family = binomial("logit"),
  control = glmerControl(optimizer = "bobyqa") 
)

summary(irt_growth_model)
```
...
item_params <- data.frame(
  Item = c("Item 5 (Easy)", "Item 7 (Medium)", "Item 6 (Hard)"),
  Difficulty = c(-1.79240, -0.10075, 2.02202), 
  Discrimination = c(1, 1, 1)   
)
...
grid <- left_join(grid, item_params, by = "Item")

grid$Probability <- 1 / (1 + exp(-grid$Discrimination * (grid$Theta - grid$Difficulty)))

ggplot(grid, aes(x = Theta, y = Probability, color = Item)) +
  geom_line(size = 1.2) +
  geom_vline(data = item_params, aes(xintercept = Difficulty, color = Item),
             linetype = "dashed", alpha = 0.5) +
  
  labs(title = "Item Characteristic Curves (ICCs)",
       x = "Theta (Latent Ability)",
       y = "Probability of Endorsement") +
  theme_minimal()
```

## 2-PL adds Discrimination

How well items can differentiate a person who has high theta levels from a person who has low theta levels? Related to how strongly the item is associated with the latent construct. Is it a good item? Does it have construct irrelevant content? 
...
item_discriminations <- rlnorm(n_items, meanlog = 0, sdlog = 0.25)
item_difficulties <- runif(n_items, -2, 2)
...
df <- df |>   
  dplyr::mutate(
    Theta = person_intercepts[ID] + (person_slopes[ID] * Time),
    Discrimination = item_discriminations[Item],
    Difficulty = item_difficulties[Item],
...
#| code-fold: true
brms_formula <- bf(
  Response ~ exp(logalpha) * theta + delta,
  theta ~ 1 + Time + (1 + Time | ID_Factor),
  logalpha ~ 1 + (1 | Item_Factor),
  delta ~ 1 + (1 | Item_Factor),
  nl = TRUE)


priors <- c(
  prior(normal(0, 1), nlpar = "theta"),    
  prior(normal(0, 0.5), nlpar = "logalpha"), 
  prior(normal(0, 3), nlpar = "delta"))
...
#| code-fold: true
library(tidybayes)
item_parameters <- irt_growth_2pl %>%
  spread_draws(
    b_logalpha_Intercept,
    b_delta_Intercept,
    r_Item_Factor__logalpha[Item, Term],
    r_Item_Factor__delta[Item, Term]
  ) |> 
  mutate(
    Discrimination = exp(b_logalpha_Intercept + r_Item_Factor__logalpha),
    Difficulty = b_delta_Intercept + r_Item_Factor__delta
  ) |> 
group_by(Item) %>%
  summarise(
    Mean_Alpha = mean(Discrimination),
    Mean_delta = mean(Difficulty))
...
#| code-fold: true
item_params <- data.frame(
  Item = c("Item 4 ", "Item 7", "Item 10"),
  Difficulty = c(-1.64536299, 0.01865466, 1.61749528), 
  Discrimination = c(0.6629388, 0.8400569, 0.5800594)   
)
...
- These are our first "latent" models for measuring change. Latent in the sense that there is some growth process that is hidden. 

- Latent =/= construct. We are interested in constructs, but constructs are more theoretical. Latent has to do with decreasing measurement error. 

- Observed = true + error

## Likert IRT

With likert scales we can also estimate an IRT model. We want to estimate the cutpoints where someone goes from answering a 1 to a 2 or a 2 to a 3, etc. 


![](grm1.png)


## graded response model growth model


$$\ln \left( \frac{P(Y_{ipt} \ge k)}{1 - P(Y_{ipt} \ge k)} \right) = \alpha_i (\theta_{pt} - \beta_{ik})$$ 

$\alpha_i$: The discrimination (slope) for item $i$.$\beta_{ik}$: The category threshold (difficulty) for responding $k$ or higher on item $i$.$\theta_{pt}$: The latent ability of person $p$ at time $t$.


$$\theta_{pt} = \eta_{0p} + \eta_{1p} \cdot \text{Time}_{t} + \zeta_{pt}$$


----------------

$$P(Y_{ipt} \ge k) = \frac{\exp \left[ \alpha_i \left( (\eta_{0p} + \eta_{1p} \text{Time}_{t}) - \beta_{ik} \right) \right]}{1 + \exp \left[ \alpha_i \left( (\eta_{0p} + \eta_{1p} \text{Time}_{t}) - \beta_{ik} \right) \right]}$$
