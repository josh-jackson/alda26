...
item_params <- data.frame(
  ...
  Difficulty = c(-1.79240, -0.10075, 2.02202), # The 'b' parameter
  Discrimination = c(1, 1, 1)   # The 'a' parameter (Set to 1 for Rasch)
)
...
With likert scales we can also estimate an IRT model. Will use cumulative likelihood aka ordered categorical.   

We want to estimate the cutpoints where someone goes from answering a 1 to a 2 or a 2 to a 3, etc. Otherwise similar to before. 

$$\ln\left(\dfrac{Pr(y_i)}{Pr(y_i)}\right)=\ln\left(\dfrac{Pr(y_i \leq k)}{1-Pr(y_i \leq k)}\right)= \alpha_k$$
...
b5_soep_long.item.E <- b5_soep_long.item |> filter(trait == "E")
b5_soep_long.item$value <- factor(b5_soep_long.item$value, ordered = TRUE)

b5_soep_long.item.E<- b5_soep_long.item.E |> 
   mutate(ID = Procedural__SID) |> 
  mutate(ID = as.factor(ID)) |> 
  mutate(year = as.numeric(year)) |> 
  mutate(item = as.factor(item))

bform <- bf( value ~ 1 + year + (1 | item) + (1 + year | ID), 
  disc ~ 1 + (1 | item))
