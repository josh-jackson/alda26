...
-   One thing to keep in mind is that we are now changing the meaning of the random effect. $U_{0j}$ is the person specific deviation from the group predicted intercept.
...
Similar to before, the interpretation of $U_{1i}$ changes.
...
Level 1 residual variance $${R}_{ti} \sim \mathcal{N}(0, \sigma^{2})$$

How does your residual change relative to a model without group effects?
...
# regressions
    ...
    s ~ cpeer + male
'
fit.4 <- growth(sem.4, data = alcohol.wide, missing = "ML")
summary(fit.4)
```
...
2.  Group-mean centering (our group/nesting is person so this is also called person centering).

3.  Group grand-mean centering (centering around average person mean)

4.  Grand-mean centering (this is taking the average across every obs)

5.  Centering on a value of theoretical or applied interest (we often do this for growth models)
...
-   Because level 2 is involved with cross-level interactions, it is helpful to consider centering. For level 2, the centering options are much easier, as one can generally go with grand mean centering.
...
Random effect says that the gap between exercise and not exercise differs across people.
...
"Controlling for a level 1 makes the slope individual differences clearer"

## Interactions among level 1 variables

Couldn't exercise levels influence the slope of health?
...
-   How do you interpret each of the terms?
...
alcohol <- alcohol |>
  group_by(id) |>
  mutate(happy = (.5 * cpeer) + rnorm(n(), mean = 0, sd = 1)) |>
...
fit7 <- lmer(alcuse ~  age_14 + happy + coa + (age_14 | id), data = alcohol)
summary(fit7)
```
...
fit8 <- lmer(alcuse ~  age_14 * coa +  age_14 * happy + (age_14 | id), data = alcohol)
summary(fit8)
```

------------------------------------------------------------------------

Why does this not run and/or make sense?

```{r, eval = FALSE}
fit8 <- lmer(alcuse ~  age_14 * coa +  age_14 * happy + (age_14 + age_14 * happy | id), data = alcohol)
```

## SEM TVCs

Level 1 variables (tvs) are flexibly handled in SEM. Whereas previously a level 1 variable had a constant effect (ie you just had a single parameter) here you can model separate effects for different times. Or you can have a tvc for one timepoint but not another.
...
For a recent paper we coded a life event as a tvc, which effectively switched on (and maybe off) when the person encountered a life event.
...
| Variable | Definition | Value for Control Group | Value for Divorce Group |
|:-----------------|:-----------------|:-----------------|:-----------------|
| **Time** | General time clock (centered at event year). | -2, -1, 0, 1, 2 | -2, -1, 0, 1, 2 |
| **Divorced** | Static Group Dummy. | 0 | 1 |
| **Acute_Event** | Time-varying dummy | Always 0 | 0 before, 1 after |
| **Time_Since** | Time-varying slope | Always 0 | 0 before, 1, 2, 3... after |

------------------------------------------------------------------------

$$\begin{aligned}
Happiness_{ti} = &\gamma_{00} \text{ (Intercept: Controls at Time 0)} \\
&+ \gamma_{01} (\text{DivorceGroup}_i) \text{ (Baseline difference for future divorcées)} \\
&+ \gamma_{10} (\text{Time}_{ti}) \text{ (Slope for Controls)} \\
&+ \gamma_{11} (\text{DivorceGroup}_i \times \text{Time}_{ti}) \text{ (Slope difference for Pre-Divorce group)} \\
&+ \gamma_{20} (\text{AcuteEvent}_{ti}) \text{ (Immediate drop at moment of divorce)} \\
&+ \gamma_{30} (\text{TimeSince}_{ti}) \text{ (Change in slope (recovery) after divorce)} \\
&+ e_{ti}
\end{aligned}$$
...
n_per_group <- 50
ids <- factor(1:(2 * n_per_group))
groups <- c(rep("Control", n_per_group), rep("Divorced", n_per_group))
base_happiness <- rnorm(2 * n_per_group, mean = 7, sd = 1) 

subjects <- data.frame(ID = ids, Group = groups, Base_Y = base_happiness)

data_long <- expand.grid(ID = ids, Time = -5:5) %>%
  left_join(subjects, by = "ID")


data_sim <- data_long %>%
  mutate(
    Is_Divorced = ifelse(Group == "Divorced", 1, 0),
    Acute_Event = ifelse(Is_Divorced == 1 & Time >= 0, 1, 0),
    Time_Since = ifelse(Is_Divorced == 1 & Time > 0, Time, 0),
    Happiness = Base_Y + 
                (-0.09 * Time) +                      
                (-0.17 * Is_Divorced * Time) +        
                (-1.50 * Acute_Event) +              
                (0.40 * Time_Since) +                 
                rnorm(n(), 0, 0.5)                    
  )

model_divorce <- lmer(Happiness ~ Time * Is_Divorced + 
                      Acute_Event + Time_Since + (1 | ID), 
                      data = data_sim)


data_sim$Predicted <- predict(model_divorce)

ggplot(data_sim, aes(x = Time, y = Predicted, color = Group)) +
  geom_line(aes(group = ID), alpha = 0.1) +
  stat_summary(fun = mean, geom = "line", size = 2) +
  geom_vline(xintercept = 0, linetype = "dashed") +
  annotate("text", x = 0.5, y = 8, label = "Event Occurs", hjust = 0) +
  labs(y = "Happiness Score", x = "Years (Centered on Event)") +
  theme_minimal()
```
...
Intercept: Control group at Time 0

Is_Divorced: Selection effect, pre existing differences

Time: Rate of change in control

Time:Is_Divorced: Pre-event slope difference

Acute_Event: "Switch" effect of divorce

Time_Since: Change in slope after divorce event

------------------------------------------------------------------------

All of our information of the trajectory is there, but the hypotheses we have may not be encoded in the parameters.

Control Slope = Time

Divorce Pre-Slope = Time + Time:Is_Divorced

Divorce Post-Slope = Time + Time:Is_Divorced + Time_Since

?? = Time:Is_Divorced + Time_Since

------------------------------------------------------------------------

What is the trajectory of the Divorced Group after the divorce?

```{r}
#| code-fold: true
library(marginaleffects)
hypotheses(model_divorce, "Time + Time:Is_Divorced + Time_Since = 0")

```

------------------------------------------------------------------------

Is the trajectory of the Divorced Group after the divorce significantly different from the trajectory of the Control Group?

```{r}
#| code-fold: true
hypotheses(model_divorce, "Time:Is_Divorced + Time_Since = 0")
```

------------------------------------------------------------------------

Notice how the difference in the post divorce trajectory plus the control trajectory = the divorce groups trajectory.
...
What if we had the switch turn on and then off?

$$\begin{aligned}
Happiness_{ti} = &\gamma_{00} \text{ (Intercept)} \\
&+ \gamma_{01} (\text{DivorceGroup}_i) \text{ (Baseline difference)} \\
&+ \gamma_{10} (\text{Time}_{ti}) \text{ (Control Slope)} \\
&+ \gamma_{11} (\text{DivorceGroup}_i \times \text{Time}_{ti}) \text{ (Difference in Slope)} \\
&+ \gamma_{20} (\text{Switch}_{ti}) \text{ (Temporary at Time 0 ONLY)} \\
&+ e_{ti}
\end{aligned}$$
...
#| code-fold: true
set.seed(123)
data_full <- expand.grid(ID = factor(1:100), Time = -3:3) %>%
  mutate(
    Is_Divorced = ifelse(as.numeric(ID) > 50, 1, 0),
    Acute_Pulse = ifelse(Is_Divorced == 1 & Time == 0, 1, 0),
    Time_Since = ifelse(Is_Divorced == 1 & Time > 0, Time, 0),
    Happiness = 7 + 
                (-0.5 * Is_Divorced) +         
                (-0.1 * Time) +                
                (-0.2 * Is_Divorced * Time) +   
                (-2.0 * Acute_Pulse) +          
                (0.6 * Time_Since) +           
                rnorm(n(), 0, 0.5)
  )

model_full <- lmer(Happiness ~ Time * Is_Divorced + Acute_Pulse + Time_Since + (1 | ID), data = data_full)


grid <- expand.grid(Time = seq(-3, 3, by = 0.5), Is_Divorced = c(0, 1)) %>%
  mutate(
    Acute_Pulse = ifelse(Is_Divorced == 1 & abs(Time) < 0.1, 1, 0), 
    Time_Since = ifelse(Is_Divorced == 1 & Time > 0, Time, 0),
    Group_Label = ifelse(Is_Divorced == 1, "Divorce Group", "Control Group")
  )

preds <- predictions(model_full, newdata = grid, re.form = NA)

ggplot(preds, aes(x = Time, y = estimate, color = Group_Label)) +
  geom_line(size = 1.5) +
  geom_point(size = 2) +
  geom_vline(xintercept = 0, linetype = "dashed", alpha = 0.5) +
  labs(title = "Short term shock",subtitle = "The new slope starts from where you were before",
       y = "Happiness", x = "Years (Centered on Event)") +
  theme_classic() 
```

------------------------------------------------------------------------

Same number of parameters. Just different conceptualization of where post event slope occurs.

```{r}
summary(model_full)
```

## Centering level 1 not time

-   If we just leave the level 1 TVC variable by itself there is often a combination of within and between person information. The two sources are "smushed" together.

-   I could exercise more than the average person (between person) but my research question is about what happens when *I* exercise a lot. Because my level varies there is within person information.

-   It is not clear whether exercise is associated with health BECAUSE people who exercise tend to be healthier or is it because when someone exercises they feel healthier.

## person mean centering

-   Typically for level 1 we will want to within person-mean center to help with the issue of "smushed" variance.

-   However, this gets rid of all mean level information for a person. The question at hand is not whether you exercise more or less it is compared to your typical levels.
...
-   For longer term data, level 1 is often used as a covariate rather than a focal coefficient. As a result, centering becomes less important bc you are controlling for both sources of variance.

## Intensive longitudinal data

-   Data collected by EMA/ESM/daily diary etc. Typically over short periods of time.

-   Questions are often about variations and predictors of those variations rather than explicit change processes. Fluctuations vs prolonged, lasting change. Often focuses on "mechanisms" rather than charting development

-   The ideas we've been developing are no different for ILD data. These are just observations nested within individual. We can think of our typical two level model.

------------------------------------------------------------------------

level 1: $${affect}_{ti} = \beta_{0i}  + \beta_{1i}Time_{ti} + \beta_{2i}Stress_{ti} + \varepsilon_{ti}$$
...
## Time does not need to be focal
...
3.  Not focus on linear time. E.g. a dummy for week vs weekend or working vs not

------------------------------------------------------------------------

level 1: $${affect}_{ti} = \beta_{0i}  + \beta_{1i}Stress_{ti} + \varepsilon_{ti}$$

Level 2: $${\beta}_{0i} = \gamma_{00} +   U_{0i}$$

$${\beta}_{1i} = \gamma_{10} +  U_{1i}$$

## Seperating between and within
...
$${affect}_{ti} = \beta_{0i} + \beta_{1i}(Stress_{ti}-\overline{Stress_{i}}) + \varepsilon_{ti}$$

Level 2: $${\beta}_{0i} = \gamma_{00} + \gamma_{01}\overline{Stress_{i}} + U_{0i}$$
...
$${\beta}_{1i} = \gamma_{10} + \gamma_{11}\overline{Stress_{i}} + U_{1i}$$

## Multiple level 1 predictors

$${affect}_{ti} = \beta_{0i} + \beta_{1i}Exercise_{i} +  \beta_{2i}(Stress_{ti}-\overline{Stress_{i}}) + \varepsilon_{ti}$$
...
What would change if we did or did not center exercise?

------------------------------------------------------------------------

-   For longer term data, where the focus is on a trajectory, and the factors that impact it, controlling for TVC like exercise often is done level 1 but not at level 2.
...
-   So if you want to interpret the within person effect of exercise, then you need to person center and put the mean effect back intothe model.

## What happens if we want to add a level 2 predictor?

$${affect}_{ti} = \beta_{0i} + \beta_{1i}Exercise_{i} +  \beta_{2i}(Stress_{ti}-\overline{Stress_{i}}) + \varepsilon_{ti}$$ Can you interpret each of these $\gamma_{02}$, $\gamma_{12}$ , $\gamma_{22}$ terms at L2?

## What happens if we want to add a level 2 predictor?

Level 2: $${\beta}_{0i} = \gamma_{00} + \gamma_{01}\overline{Stress_{i}} + \gamma_{02}Age_i + U_{0i}$$

$${\beta}_{1i} = \gamma_{10} + \gamma_{11}\overline{Stress_{i}} + \gamma_{12}Age_i +  U_{1i}$$ $${\beta}_{2i} = \gamma_{20} + \gamma_{22}Age_i + U_{2i}$$

## Lagged models

What if I want to predict something in the future?

$${affect}_{ti} = \beta_{0i} + \beta_{1i}(Stress_{(t-1)i}-\overline{Stress_{i}}) + \varepsilon_{ti}$$
...
Should I control for current time?

$${affect}_{ti} = \beta_{0i} + \beta_{1i}(Stress_{(t-1)i}-\overline{Stress_{i}}) +\beta_{2i}(Stress_{ti}-\overline{Stress_{i}}) + \varepsilon_{ti}$$

Level 2: $${\beta}_{0i} = \gamma_{00} + \gamma_{01}\overline{Stress_{i}} + U_{0i}$$

$${\beta}_{1i} = \gamma_{10} + U_{1i}$$ $${\beta}_{2i} = \gamma_{10} + U_{2i}$$

## Contextual effect

-   Person (Group) mean centering provides us with coefficients for within and between person effect that are not correlated.

-   What would happen if we just left the level 1 raw but controlled for average levels? 

## Contextual effect

$${affect}_{ti} = \beta_{0i} + \beta_{1i}(Stress_{ti}) + \varepsilon_{ti}$$
...
- Highly correlated coefficients but you are controlling for the mean, so the within person effect $\gamma_{10}$ is interpreted as the same.

- Said differently: if you only care about the within person effect, then you do not need to necessarily person mean center. 

- But... What does  $\gamma_{01}$ test in this case? What did it test previously? 

## Contextual effect

-   We might want to ask is the within person effect similar in magnitude to the between person effect. Is it more important to not be a stressful person or is it more important to not be stressed during a particular time?

-   $\gamma_{01}$ tests the difference between the between and the within effect. Positive means the between person trait effect is stronger than the within person state effect.

-   Being a stressed person is worse for you than just having a stressful day.
...
#| code-fold: true
set.seed(123)
subjects <- data.frame(ID = factor(1:10), 
                       True_Mean_X = seq(2, 8, length.out = 10),
                       Slope_Deviation = rnorm(10, mean = 0, sd = 0.8))


df_viz <- expand.grid(ID = factor(1:10), Time = 1:15) %>%
  left_join(subjects, by = "ID") %>%
  group_by(ID) %>%
  mutate(
    X_val = True_Mean_X + rnorm(10, 0, 1),
    Y_val = 10 + 
            ((0.7 + Slope_Deviation) * (X_val - True_Mean_X)) + 
            (1 * True_Mean_X) +                               
            rnorm(10, 0, 0.5)                                   
  ) %>%
  ungroup()
...
df_means <- df_viz %>%
  group_by(ID) %>%
  summarize(Mean_X = mean(X_val), Mean_Y = mean(Y_val))


grand_mean_x <- mean(df_viz$X_val)
grand_mean_y <- mean(df_viz$Y_val)
fixed_slope <- 0.5
fixed_intercept <- grand_mean_y - (fixed_slope * grand_mean_x)

ggplot() +
  geom_smooth(data = df_viz, aes(x = X_val, y = Y_val, group = ID, color = ID), 
              method = "lm", se = FALSE, size = 0.5, alpha = 0.5) +
  geom_point(data = df_means, aes(x = Mean_X, y = Mean_Y, color = ID), size = 3) +
  geom_smooth(data = df_means, aes(x = Mean_X, y = Mean_Y), 
              method = "lm", color = "black", se = FALSE, size = 1.5, linetype = "dashed") +

  
  labs(title = " Dashed = Between Slope | Colored = Random Slopes",
       x = "Stress", y = " Affect") +
  theme_classic() +
  theme(legend.position = "none")
```
...
ggplot() +
  ...
  labs(title = "Blue = Average Within Slope | Dashed = Between Slope | Colored = Random Slopes",
       x = "Stress", y = " Affect") +
  theme_classic() +
  theme(legend.position = "none")
```
...
-   Typical, for contextual effects, we grand mean center the level 1 rather than leave it raw. The interpretation of the state and trait coefficients do not change, but the intercept may be uninterpretable.

-   Grand mean centering does not change the interpretation of the contextual effect but helps with interpretation.

------------------------------------------------------------------------

L1 centered at grand mean.

$${affect}_{ti} = \beta_{0i} + \beta_{1i}(Stress_{ti}-\overline{Stress_{..}})  + \varepsilon_{ti}$$

Level 2: $${\beta}_{0i} = \gamma_{00} + \gamma_{01}\overline{Stress_{i}} -\overline{Stress_{..}} + U_{0i}$$

$${\beta}_{1i} = \gamma_{10} + U_{1i}$$


## centering and random effects Level 1

-   For PMC, the effect is relative association compared to typical person centered effect -- maybe some people have an effect where deviations of stress influences affect but others these deviations do not.

-   For GMC, the effect contains both within and between person effects, so the interpretation is messier. The fixed effect assumes people are at the grand mean. Are they happier than the at the average level of stress?
...
- $$U_{0i}$$ For PMC, the effect represents intercept differences when people are at their person mean. Are they happier than the avg person, at their avg level of stress? For GMC, the effect when people are at the grand mean. Are they happier than the average person at the average person's level of stress?

- Just be careful in interpretation if these values (person mean vs grand mean) can differ a lot.  
...
- You can keep raw tvc at level 1 but I'd  include the person mean at level 2 so as to get the unconfounded within-person effect. Just be clear that the interpretation of level 2 is now the contextual effect.   


## MELSM

- We make a huge assumption in standard ILD models. That people's variability is the same. 

- Mixed effects location (mean) scale (variance) model. Basically it means modeling multiple distributional parameters simultaneously. Anticipating our multivariate models in the next slide deck. 
