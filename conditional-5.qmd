...
-   What would happen if we just left the level 1 raw? Highly correlated coefficients but you are controlling for the mean, so the within person effect $\gamma_{10}$ is interpreted as the same.

-   What does "\gamma\_{01}" test in this case?
...
-   "\gamma\_{01}" tests the difference between the between and the within effect. Positive means the between person trait effect is stronger than the within person state effect.

-   Being a stressed person is worse for you than just having a stressful day.

## Contextual effect

-   Typical we grand mean center the level 1 rather than leave it raw. The interpretation of the state and trait coefficients do not change, but the intercept may be uninterpretable.

-   Grand mean centering does not change the interpretation of the contextual effect and helps with interpretation.
