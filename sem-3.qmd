...
Usually done through chi-square difference test. You want to use the "Model Test User Model"

It compares the model predicted covariance matrix against the actual covariance matrix. 

Null is your model fits data perfectly. Alternative is that it does not. Thus we want a non significant chi square test. 

## Comparing the models

Likelihood Ratio Test (LRT) aka  the Chi-Square Difference Test

$$D = \chi^2_{\text{simple}} - \chi^2_{\text{complex}}$$
$$df_{\text{diff}} = df_{\text{simple}} - df_{\text{complex}}$$
You want NS, i.e. the constrains do not lead to worse fit. 
...
But this LRT is a very sensitive test, especially with larger samples. Better to look at changes in CFI. If delta is .01 or greater than maybe it shows misfit.
