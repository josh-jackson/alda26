...
$$ y =  \Lambda *  \boldsymbol{\eta_i} + \epsilon_i $$ The model-implied covariance matrix ($\boldsymbol{\Sigma}$) can be decomposed into two distinct matrices:

$$\boldsymbol{\Sigma} = \boldsymbol{\Lambda} \boldsymbol{\Phi} \boldsymbol{\Lambda}' + \boldsymbol{\Theta}$$ 
$\boldsymbol{\Lambda} \boldsymbol{\Phi} \boldsymbol{\Lambda}'$ (Common Variance): This matrix represents the variance shared among items—the "signal." $\boldsymbol{\Theta}$ (Unique Variance): This matrix (usually diagonal) represents the measurement error

The model effectively "subtracts" the noise from the total variance. The Structural Model (the regressions between latent factors) then operates exclusively on $\boldsymbol{\Phi}$ (the covariance of the true scores)
...
-   Largely irrelevant as to what scale is chosen as they are algebraic equivalent, similar to centering or standardizing.
...
-   Given we are interested in change across time, we will be interested in means. Latent variables by themselves do not have any inherent metric, it is up to us to choose the scale they are on.
