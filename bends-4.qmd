...
Logistic function from before: 
$$P(Y=1) = \frac{1}{1 + e^{-b_0 + b_1X}}$$

Same function, but the max is now asymptote rather than 1. 

$$f(t) = \frac{\text{asymptote}}{1 + e^{((\text{mid}~-~t)~*~\text{scale})}}$$
Scale is the slope, much like B1. t is our predictor. We use midpoint to mark where curve is centered. (For the IRT folk, mid and scale are basically our discrimination and difficulty parameters)
...
```{r}
inv_logit(1.46) ## Asymptote 
```
```{r}
inv_logit(-1.96 ) ## scale at midpoint
```
...
We just saw a nonlinear growth model with the logistic function. But there are other functions that are similar to the standard logistic: 
- Gompertz Curve (accelerates quickly but slows down, inflection occurs earlier thant logistic)
- Log-Logistic (aysmetrical S, assumes a "Heavy Tail")
- Von Bertalanffy (starts fast, no S)
...
$$ asym \left(1 - \exp\left(- \left( \frac{t}{\theta} \right)^\omega \right)
\right)$$ 
...
## Exponential family of growth curves
...
$$Y_{ti}=\ a_i-\left(a_i-\ b_i\right)\ast e^{\left(-c_i*{time}_{ti}\right)}\ $$
$$b_i=\ \gamma_{b}+\ U_{b{i}}$$
$$c_j=\ \gamma_{c0}+\ U_{cj}$$
$b_i$ can be thought of as the intercept, whereas $c_i$ is the slope. $a_i$ is maximum possible value
